...
| Stereo/LiDAR fusion, multi-view SfM/SLAM (camera pose) | **OPEN SOURCE — first slice done, free** | Free-tier plan step B1 (docs/free-tier-roadmap.md): `.github/workflows/estimate-camera-pose.yml` runs real COLMAP on a free GitHub Actions CPU runner, triggered from a "Estimate real camera pose" button (`src/services/camera-pose.service.ts`). Async by necessity (minutes, not seconds) — deliberately kept outside the synchronous AIJob pipeline via a dedicated `CameraPoseJob` model and a shared-secret callback, the same pattern `/api/cron/process-jobs` already established. Replaces the synthesized placeholder camera-position ring with real measured poses (`C = -Rᵀt` from COLMAP's quaternion/translation output), now visible via the new camera-pose viewer below. Needs a GitHub PAT (`GITHUB_DISPATCH_TOKEN`) — the one manual setup step. |
...
| 3D Gaussian Splatting | **OPEN SOURCE → HYBRID — storage/render slice done, free** | Free-tier plan step B2 (docs/free-tier-roadmap.md, docs/gaussian-splatting-guide.md): `src/services/splat.service.ts` stores a real, already-trained `.ply`/`.splat`/`.ksplat` file as a new `Reconstruction` version, `src/components/spaces/splat-viewer.tsx` renders it with `@mkkellogg/gaussian-splats-3d` (a maintained open-source Three.js splat renderer — reused, not built from scratch, per this row's own original reasoning). Training itself (`gsplat` on a free Colab/Kaggle GPU notebook) stays a manual step outside the app — no provider hosts splat training as a simple API call, verified while scoping this. Compression/streaming/quality tuning for our specific hospitality use case, and wiring the viewer into the *public* experience page rather than just the authenticated space detail page, remain real, ongoing **HYBRID** work. |
...
| Voice (STT/TTS) | **BUY — done, free tier** | `src/providers/voice/nvidia-voice-provider.ts` (`VOICE_PROVIDER=nvidia`), NVIDIA's hosted Riva NIMs (`parakeet-tdt-0.6b` ASR, `magpie-tts-multilingual` TTS), free tier. Wired into the public concierge as a mic input and per-answer playback. Verified with a real round trip (TTS output fed back into ASR reproduced the exact original text) and through the actual browser UI end to end, which also caught two real pre-existing bugs: a site-wide `Permissions-Policy` header silently blocking microphone access, and the CSRF middleware incorrectly blocking every public `/api/experience/**` route (including the concierge and analytics endpoints already shipped) whenever the visitor's browser happened to carry a stray session cookie. |
