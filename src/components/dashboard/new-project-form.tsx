  ...
  async function createProject() {
    ...
    try {
      const { project } = await apiFetch<{ project: { id: string } }>("/api/projects", {
        method: "POST",
        body: JSON.stringify({ name, type, organizationId }),
      });
      // Hard navigation, not router.push() — see login/page.tsx for why:
      // the same silent-no-navigation failure reproduces here too, for a
      // plain mutation-then-navigate with no cookie/auth change involved,
      // so it isn't specific to auth state as originally assumed.
      window.location.href = `/projects/${project.id}/capture`;
    } catch (err) {
